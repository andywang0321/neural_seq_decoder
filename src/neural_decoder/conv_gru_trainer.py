...
def trainModel(args):
    ...
    # convDim is optional; default to nInputFeatures if not set
        ...
        if batch % 100 == 0:
            with torch.no_grad():
                ...
                for X, y, X_len, y_len, testDayIdx in testLoader:
                    ...
                    input_lengths = ((X_len - model.kernelLen) / model.strideLen).to(
                        torch.int32
                    )
...
def loadModel(modelDir, nInputLayers=24, device="cuda"):
    ...
    # convDim might not exist in older runs; handle gracefully
    conv_dim = args.get("convDim", args["nInputFeatures"])
